...
GameOptions::GameOptions(irr::gui::IGUIEnvironment *env, irr::video::IVideoDriver *driver, irr::scene::ISceneManager *smgr)
{
  _driver = driver;
  _env = env;
  _smgr = smgr;
...
void GameOptions::loadTextures()
{
    _textures["retour"] = _driver->getTexture("resources/images/buttons/b.png");
...
void GameOptions::loadButtons()
{
    _buttons["retour"] = _env->addButton(irr::core::rect<irr::s32>(0, 0, 215, 47), nullptr, GUI_ID_PAUSE_RETURN, L"");
...
void GameOptions::run()
{}

std::vector<Character *> GameOptions::getPreviews()
{
  return {};
}

std::map<std::string, irr::gui::IGUIButton *> GameOptions::getButtons()
{
  return _buttons;
}

std::map<std::string, irr::gui::IGUIImage *> GameOptions::getImages()
{
  return _images;
}

std::map<std::string, irr::gui::IGUICheckBox *> GameOptions::getCheckBox()
{
  return _checkBox;
}