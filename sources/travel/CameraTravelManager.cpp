    ...
    switch (travel) {
        ...
        case travel::introToMenu:
            if (auto travel = getTravel(introToMenu)) {
                ...
                _camera->getPosition(), {-179.2f, 55.8f, -36.1f}, {-111.0f, 45.0f, -24.0f}, u32(6000));
                if (auto travel = getTravel(introToMenu))
                    travel->start();
                else
        ...
        case travel::menuToSelect:
            if (auto travel = getTravel(menuToSelect)) {
                ...
                {-179.2f, 55.8f, -36.1f}, {-94.3f, 44.1f, 294.3f}, {-70.0f, 21.0f, 232.0f}, u32(6000));
                if (auto travel = getTravel(menuToSelect))
                    travel->start();
                else
        ...
        case travel::selectToMenu:
            if (auto travel = getTravel(selectToMenu)) {
                ...
                {-94.3f, 44.1f, 294.3f}, {-179.2f, 55.8f, -36.1f}, {-111.0f, 45.0f, -24.0f}, u32(6000));
                if (auto travel = getTravel(selectToMenu))
                    travel->start();
                else
